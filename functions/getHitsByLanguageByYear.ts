...
export async function handler(event) {
    ...
    // Get hits by language by year sorted by hits and limited to top 10
    ...
    const query = fauna.fql`Object.entries(hits.byYear(Date.fromString(${event.queryStringParameters.date}).year).fold({}, (accumulator, value) => {
        let count = (accumulator[value.language] ?? 0) + 1
        let override = Object.fromEntries([[value.language, count]])
        Object.assign(accumulator, override)})).fold([], (acc, elm) => {
            let obj = Object.fromEntries([['language', elm.first()], ["hits", elm.last()]])
            acc.append(obj)
